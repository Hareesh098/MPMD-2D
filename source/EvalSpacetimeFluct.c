...
void EvalSpacetimeFluct(){
  ...
  for (nb = 1; nb <= nBuffCorr; nb++){
    indexFluct[nb] += 1;
    if (indexFluct[nb] <= 0) continue;
    ni = 3*nFunCorr * (indexFluct[nb] - 1);
    if (indexFluct[nb] == 1){
      for (j = 1; j <= 6*nFunCorr; j++)
       complxFuncOrg[nb][j] = complxFunc[j];
    }

    for (j = 1; j <= 3*nFunCorr; j++)
      spacetimeFluct[nb][ni + j] = 0.;
    ...
    for (m = 1; m <= nFunCorr; m++){
     ...
     for(nc = 1; nc <= 3; nc ++){
       ...
       spacetimeFluct[nb][nv] += w * ( complxFunc[j] * complxFuncOrg[nb][j] + complxFunc[j + 1] * complxFuncOrg[nb][j + 1] ) ;
      j += 2;
      }
    }
  }
