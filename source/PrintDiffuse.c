...
void PrintDiffusion(){
  int j;
  real tVal = 0. ;
  fprintf(fpdiffuse,"Diffusion by Einstein's defination\n");
  for(j = 1; j <= nValAcf; j ++){
  tVal = (j - 1) * stepAcf * deltaT ;
  fprintf(fpdiffuse, "%lf\t%lf\t%lf\t%lf\t%lf\n", tVal, xDiffuseAv[j], yDiffuseAv[j], rDiffuseAv[j], MSDAv[j]);
  }
  fprintf(fpdiffuse, "Diffusion constant at time = %lf  ,by Einstein's defination is  : %lf\n", tVal,  rDiffuseAv[nValAcf]);
  }								
